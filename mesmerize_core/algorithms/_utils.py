...
class ColumnMappingFunction(Generic[R]):
    ...
    def _helper(self, args: tuple) -> R:
        ...
        mov: cm.movie = cm.load(
            movie_path, subindices=subindices, var_name_hdf5=var_name_hdf5
        )
        T, *dims = mov.shape
    ...
    def __call__(
        ...
        chunk_col_slices = [
            slice(start, end) for start, end in pairwise(chunk_col_edges)
        ]
        ...
        if (n_row_chunks := math.ceil(n_chunks / n_column_chunks)) > 1:
            ...
            chunk_row_slices = [
                slice(start, end) for start, end in pairwise(chunk_row_edges)
            ]
        else:
...
def save_c_order_mmap_parallel(
    movie_path: str, base_name: str, dview: Optional[Cluster], var_name_hdf5="mov"
) -> str:
