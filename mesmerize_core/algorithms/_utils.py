...
def fix_multid_subindices(
    movie_path: str, subindices: Union[list, tuple]
) -> Union[list, tuple]:
    """
...
class ColumnMappingFunction(Generic[R]):
    ...
    def _helper(self, args: tuple) -> R:
        ...
        if page is not None:
            subindices += (page,)
            logging.debug(
                f"In column mapping kernel, page = {page}, cols = {col_slice.start} to {col_slice.stop}"
            )
        else:
            logging.debug(
                f"In column mapping kernel, cols = {col_slice.start} to {col_slice.stop}"
            )

        mov: cm.movie = cm.load(
            movie_path,
            subindices=fix_multid_subindices(movie_path, subindices),
        ...
        pixel_slice = slice(
            page_offset + col_slice.start * dims[0] + row_slice.start,
    ...
    def __call__(
        self, movie_path: str, dview: Optional[Cluster], var_name_hdf5="mov", *args
    ) -> Iterable[R]:
        ...
        chunk_col_slices = [
            slice(start, end)
            for start, end in zip(chunk_col_edges[:-1], chunk_col_edges[1:])
        ]

        if (n_row_chunks := math.ceil(n_chunks / n_column_chunks)) > 1:
            # subdivide rows as well
            chunk_row_edges = [
                int(x) for x in np.linspace(0, dims[0], n_row_chunks + 1)
            ]
            chunk_row_slices = [
                slice(start, end)
                for start, end in zip(chunk_row_edges[:-1], chunk_row_edges[1:])
            ]
        else:
        ...
        map_args = [
            ...
            for page in pages
            for col_slice in chunk_col_slices
            for row_slice in chunk_row_slices
        ]
...
def _save_c_order_mmap_in_chunks_kernel(
    ...
    filter_sos: Optional[np.ndarray] = None, filter_padtype: Optional[Literal['odd', 'even', 'constant']] = 'odd',
    filter_padlen: Optional[int] = None
):
    """
...
def save_c_order_mmap_parallel(
    movie_path: str,
    base_name: str,
    dview: Optional[Cluster],
    fr: float,
    ...
    save_c_order_mmap_in_chunks(
        ...
        output_path, byte_offset, valid_mask, add_to_movie, add_to_movie_temporal,
        filter_sos, highpass_padtype, highpass_padlen
    )
...
def _make_chunk_projections_kernel(
    Yr_chunk: np.ndarray, _pixel_slice: slice, proj_type: str, ignore_nan=False
) -> np.ndarray:
...
def make_projection_parallel(
    ...
    chunk_projs = make_chunk_projections(
        movie_path, dview, var_name_hdf5, proj_type, ignore_nan
    )
...
def make_correlation_parallel(
    movie_path: Union[str, Path], dview: Optional[Cluster]
) -> np.ndarray:
    """
...
def chunk_correlation_helper(args: tuple[str, ChunkDims]) -> np.ndarray:
    ...
    mov = cm.load(
        movie_path,
        subindices=fix_multid_subindices(movie_path, (slice(None),) + dims_input),
    )
...
def make_correlation_patches_for_dim(
    dim: int, window_size: int
    ...
    patch_starts = range(
        0, dim - overlap, stride
    )  # last <overlap> pixels are covered by last window
